---
title: Does language control differ between younger and older adults when switching
  between versus within languages?
...
packagelist <- c('lme4','lmerTest','tidyverse','ggplot2','rmarkdown','bookdown','kableExtra','rstatix')
...
tableS1mixswitchSD[1,1] <- 100*sd(means_LS_acc_allcombs_pivot$SwitchingCostYoungerObject, na.rm=TRUE) 
...
tableS3mixswitchSD[1,1] <- 100*sd(means_NLS_acc_allcombs_pivot$SwitchingCostYoungerSIZE, na.rm=TRUE) 
...
#get participant mean per rule for mixing analysis
means_NV_Exp2_rule_mix <- NounVerb_Exp2_acc_outliers %>% 
  group_by(participant, Rule) %>% 
  summarise(meanRT = mean(rawrt))
...
knitr::kable(table4data, booktabs = T, caption='Outcome of the linear mixed effect models for the bilingual switching task in Experiment 2, The final switching model included all by-subject and by-item random intercepts and slopes, after removal of correlations. The final mixing model included by-subject and by-item random intercepts and slopes, after removal of correlations.', align='lrrrr') %>%
  row_spec(c(0,1,10),bold=T) %>%
  row_spec(c(1,10),italic=T)
```
...
NVMix = ggplot(means_NVMix_Exp2_long, aes(x = Agegroup, y = NounVerbMixing)) + geom_boxplot() +
  labs(title="C. Noun-verb mixing cost",x="Age group", y = "mixing cost (ms)")+
  theme(axis.text.x = element_text(color = "black", size = 12, face = "plain"),
        axis.text.y = element_text(color = "black", size = 12, face = "plain"),
        axis.title.x = element_text(color = "black", size = 14, face = "bold"),
        axis.title.y = element_text(color = "black", size = 14, face = "bold"),
...
knitr::kable(table6data, booktabs = T, caption='Outcome of the linear mixed effect models for the noun-verb switching task in Experiment 2. The final switching model included all by-subject and by-item random intercepts and slopes, after removal of the item slope for switching x age x rule. The final mixing model included all intercepts and slopes after removal of correlations.', align='lrrrr') %>%
  row_spec(c(0,1,10),bold=T) %>%
  row_spec(c(1,10),italic=T)
```
...
17.	Gollan, T. H., Montoya, R. I., Cera, C. & Sandoval, T. C. More use almost always means a smaller frequency effect: Aging, bilingualism, and the weaker links hypothesis. *J. Mem. Lang.* **58**, 787–814 (2008).
18.	Ivanova, I., Murillo, M., Montoya, R. I. & Gollan, T. H. Does bilingual language control decline in older age? *Linguist. Approaches Biling.* **6**, 86–118 (2016).
19.	Hernandez, A. E. & Kohnert, K. J. Aging and language switching in bilinguals. *Aging Neuropsychol. Cogn.* **6**, 69–83 (1999).
20.	Weissberger, G. H., Wierenga, C. E., Bondi, M. W. & Gollan, T. H. Partially overlapping mechanisms of language and task control in young and older bilinguals. *Psychol. Aging* **27**, 959 (2012).
21.	de Bruin, A., Samuel, A. G. & Duñabeitia, J. A. Examining bilingual language switching across the lifespan in cued and voluntary switching contexts. *J. Exp. Psychol. Hum. Percept. Perform.* **46**, 759 (2020).
22.	Hernandez, A. E. & Kohnert, K. J. Investigations into the locus of language-switching costs in older adult bilinguals. *Biling. Lang. Cogn.* **18**, 51–64 (2015).
23.	Calabria, M., Branzi, F. M., Marne, P., Hernández, M. & Costa, A. Age-related effects over bilingual language control and executive control. *Biling. Lang. Cogn.* **18**, 65–78 (2015).
24.	Levelt, W. J., Roelofs, A. & Meyer, A. S. A theory of lexical access in speech production. *Behav. Brain Sci.* **22**, 1–38 (1999).
25.	Roelofs, A. A spreading-activation theory of lemma retrieval in speaking. *Cognition* **42**, 107–142 (1992).
26.	Howard, D., Nickels, L., Coltheart, M. & Cole-Virtue, J. Cumulative semantic inhibition in picture naming: Experimental and computational studies. *Cognition* **100**, 464–482 (2006).
27.	Roelofs, A. How attention controls naming: Lessons from Wundt 2.0. *J. Exp. Psychol. Gen.* **150**, 1927 (2021).
28.	Roelofs, A. Goal-referenced selection of verbal action: modeling attentional control in the Stroop task. *Psychol. Rev.* **110**, 88 (2003).
29.	Piai, V., Roelofs, A., Acheson, D. J. & Takashima, A. Attention for speaking: domain-general control from the anterior cingulate cortex in spoken word production. *Front. Hum. Neurosci.* **7**, 832 (2013).
30.	Ivanova, I. & Hernandez, D. C. Within-language lexical interference can be resolved in a similar way to between-language interference. *Cognition* **214**, 104760 (2021).
31.	Declerck, M., Ivanova, I., Grainger, J. & Duñabeitia, J. A. Are similar control processes implemented during single and dual language production? Evidence from switching between speech registers and languages. *Biling. Lang. Cogn.* **23**, 694–701 (2020).
32.	Liu, C. et al. Symmetries of bilingual language switch costs in conflicting versus non-conflicting contexts. *Biling. Lang. Cogn.* **22**, 624–636 (2019).
33.	Yahya, M. & Özkan Ceylan, A. Interactions between language and inhibitory control: Evidence from a combined language switching and Stroop paradigm. *Int. J. Biling.* **26**, 675–694 (2022).
34.	Williams, G. P., Kirk, N. W., Sánchez, M., Afshar, Z. & Wen, Y. Shared or separate: Control processes of cross-and within-language interference. (2023). doi: 10.31234/osf.io/f82wk.
35.	Verhaeghen, P., Steitz, D. W., Sliwinski, M. J. & Cerella, J. Aging and dual-task performance: a meta-analysis. *Psychol. Aging* **18**, 443 (2003).
36.	Verhaeghen, P. Aging and executive control: Reports of a demise greatly exaggerated. *Curr. Dir. Psychol. Sci.* **20**, 174–180 (2011).
37.	Wasylyshyn, C., Verhaeghen, P. & Sliwinski, M. J. Aging and task switching: a meta-analysis. *Psychol. Aging* **26**, 15 (2011).
38.	Calabria, M., Hernández, M., Branzi, F. M. & Costa, A. Qualitative differences between bilingual language control and executive control: Evidence from task-switching. *Front. Psychol.* **2**, 399 (2012).
39.	Declerck, M., Grainger, J., Koch, I. & Philipp, A. M. Is language control just a form of executive control? Evidence for overlapping processes in language switching and task switching. *J. Mem. Lang.* **95**, 138–145 (2017).
40.	Brysbaert, M. & Stevens, M. Power analysis and effect size in mixed effects models: A tutorial. *J. Cogn.* **1**, (2018).
41.	Duñabeitia, J. A. et al. MultiPic: A standardized set of 750 drawings with norms for six European languages. *Q. J. Exp. Psychol.* **71**, 808–816 (2018).
42.	Heikoop, K. W., Declerck, M., Los, S. A. & Koch, I. Dissociating language-switch costs from cue-switch costs in bilingual language switching. *Biling. Lang. Cogn.* **19**, 921–927 (2016).
43.	Anwyl-Irvine, A. L., Massonnié, J., Flitton, A., Kirkham, N. & Evershed, J. K. Gorilla in our midst: An online behavioral experiment builder. *Behav. Res. Methods* **52**, 388–407 (2020).
44.	Protopapas, A. Check Vocal: A program to facilitate checking the accuracy and response time of vocal responses from DMDX. *Behav. Res. Methods* **39**, 859–862 (2007).
45.	Grange, J.A.  trimr: An implementation of common response time trimming methods. R package version 1.0.1. (2015). https://cran.r-project.org/web/packages/trimr/index.html
46.	Philipp, A. M., Gade, M. & Koch, I. Inhibitory processes in language switching: Evidence from switching language-defined response sets. *Eur. J. Cogn. Psychol.* **19**, 395–416 (2007).
47.	Kiesel, A. et al. Control and interference in task switching—A review. *Psychol. Bull.* **136**, 849 (2010).
48.	Lemhöfer, K. & Broersma, M. Introducing LexTALE: A quick and valid lexical test for advanced learners of English. *Behav. Res. Methods* **44**, 325–343 (2012).
49.	Brysbaert, M. Lextale_FR a fast, free, and efficient test to measure language proficiency in French. *Psychol. Belg.* **53**, 23–37 (2013).
